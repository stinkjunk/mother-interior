...
export default function Images({
  ...
  return (
    <div className={className}>
      <div className="max-sm:mx-5 mx-25 md:mx-0 md:ml-25 relative aspect-square">
        {priceInt ? (
          <div className=" -ml-25 absolute bottom-0 mb-15 md:mb-20 left-0 px-3 pr-2 pl-30 text-2xl font-label z-10 filterTag border-2 border-l-0 flex">
            {isSold ? (
              `${soldLabel}`
            ) : (
              <>
        ...
        {urls.length > 1 && (
          <div className="absolute w-full h-full bottom-0 flex justify-evenly items-end px-5 md:px-10">
            <div className="absolute left-0 max-sm:h-full sm:bottom-0 h-full flex">
              <button
                onClick={handlePrev}
                className="border-2 h-7 w-7 max-sm:h-10 max-md:w-8 ml-5 max-sm:my-auto sm:mt-auto sm:mb-5 transition-all imgNavBtn"
              >
                <MdArrowLeft
                  size={1}
            ...
            <div className="absolute right-0 max-sm:h-full sm:bottom-0 h-full flex">
              <button
                onClick={handleNext}
                className="border-2 h-7 w-7 max-sm:h-10 max-md:w-8 mr-5 max-sm:my-auto sm:mt-auto sm:mb-5 transition-all imgNavBtn"
              >
                <MdArrowRight
                  size={1}
                  className="h-full w-full transition-all"
